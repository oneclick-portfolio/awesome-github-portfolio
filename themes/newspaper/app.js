...
function initializePage() {
  ...
  $$('[data-name]').forEach((el) => {
    ...
    el.textContent = el.classList.contains('np-name') ? value.toUpperCase() : value;
  });
  $$('[data-headline]').forEach((el) => {
    el.textContent = basics.headline || '';
  });
  ...
  $('#profilesGrid').innerHTML = profiles.map((profile) => {
    ...
    return `<article class="np-expertise-card"><div class="card-title">${title}</div><div class="card-body">${profile.username || ''}</div>${link ? `<div class="card-actions"><a href="${link}" target="_blank" rel="noopener" class="np-project-link">Open</a></div>` : ''}</article>`;
  }).join('');
  ...
  window.RxResumeData.getItems(resume, 'experience').forEach((item) => {
    ...
    card.className = 'np-timeline-item';
    card.innerHTML = `<div class="np-timeline-head"><div class="np-timeline-title">${item.position || ''} · ${company}</div><div class="np-timeline-meta">${item.period || ''}</div></div><div class="np-timeline-body">${item.description || ''}</div>`;
  ...
  window.RxResumeData.getItems(resume, 'projects').forEach((project) => {
    ...
    const card = document.createElement('article');
    card.className = 'np-project-card';
    card.innerHTML = `<div class="np-project-title">${project.name || ''}</div><div class="np-project-desc">${project.description || ''}</div>${link ? `<a href="${link}" target="_blank" rel="noopener" class="np-project-link">Open</a>` : ''}`;
  ...
  window.RxResumeData.getItems(resume, 'languages').forEach((lang) => {
    ...
    row.className = 'np-language';
    row.innerHTML = `<span class="np-language-name">${lang.language || ''}</span>${lang.fluency ? `<span class="np-language-fluency">${lang.fluency}</span>` : ''}`;
...
async function loadProfileArt() {
  ...
  try {
    ...
    if (pictureUrl) {
      ...
      container.style.width = (pictureMeta.size || 150) + 'px';
